...
# *************************************************************************************
            ...
            while True:
                ...
                for i in range(0, len(valid_matchesQ)):
                    if valid_matchesQ[i].baseCycle == valid_matchesQ[i].endCycle:
                        ...
                        if round(valid_matchesQ[i].data_sent) -  (valid_matchesQ[i].size+16) != 0:
                            if(valid_matchesQ[i].rankS != valid_matchesQ[i].rankR ):
                                print(str(valid_matchesQ[i].rankS) + " -->" + str(valid_matchesQ[i].rankR))
                                print("sent: " + str(valid_matchesQ[i].data_sent) +  " --- size " + str(valid_matchesQ[i].size + 16))
                                print(round(valid_matchesQ[i].data_sent) -  (valid_matchesQ[i].size+16))
                                print(valid_matchesQ[i].send_origin + " --- " + valid_matchesQ[i].recv_origin)
                                #assert False, "send data was different from original data size"
                        #print(round(valid_matchesQ[i].data_sent) -  (valid_matchesQ[i].size+16))
                        #print(valid_matchesQ[i])
                        #print(bcolors.ENDC, end='');
                        readyMatch = None;
    ...
    def __alg_SC_CC(self, matchQ, col_matchQ, currentPosition):
    ...
    def processContention(self, matchQ, col_matchQ, currentPosition):
        ...
        if (self.topology == "SC_CC"):
            return self.__alg_SC_CC(matchQ, col_matchQ, currentPosition);
